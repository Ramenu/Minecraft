...
class Shader
{
    public:
        ...
        inline void setMat4(const char *name, const glm::mat4 &matrix) const {
            glUniformMatrix4fv(glGetUniformLocation(shaderProgram, name), 1, GL_FALSE, &matrix[0][0]);
        }
        inline void setMat3(const char *name, const glm::mat3 &matrix) const {
            glUniformMatrix3fv(glGetUniformLocation(shaderProgram, name), 1, GL_FALSE, &matrix[0][0]);
        }
        inline void setVec2(const char *name, const glm::vec2 &vec)  const {
            glUniform2f(glGetUniformLocation(shaderProgram, name), vec.x, vec.y);
        }
        inline void setVec3(const char *name, const glm::vec3 &vec) const {
            glUniform3f(glGetUniformLocation(shaderProgram, name), vec.x, vec.y, vec.z);
        }
        inline void setInt(const char *name, int32_t value) const {
